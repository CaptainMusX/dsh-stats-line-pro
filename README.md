...
- OpenCode Go 路由显示 5 小时、7 天、1 个月窗口用量和重置倒计时；
...
`src/format.js` 是无 DOM 的格式化和测试单元；`src/index.js` 是主机侧白名单查询路由；`src/client.js` 负责 DSH `conversation.composer.dock` 插槽和透明多行渲染。构建产物在 `lib/`，可直接被 DSH 的插件加载器使用。
