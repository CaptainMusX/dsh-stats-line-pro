...
var StatsLinePro = (0, import_react.memo)(function StatsLinePro2({ useSession, useProjection, sessionId }) {
  ...
  const identity = (0, import_react.useMemo)(() => latestProviderOf(nodes), [nodes]);
  ...
  const quotaText = (id, payload) => {
    ...
    return view.kind === "subscription" ? view.label + " \u8BA2\u9605\u4F59\u91CF\uFF1A" + view.windows.map((w) => w.label + " \u5269\u4F59 " + (Number.isFinite(w.percent) ? Math.max(0, 100 - w.percent) + "%" : "\u672A\u77E5") + "\uFF08" + w.countdown + " \u540E\u91CD\u7F6E\uFF09").join(" \xB7 ") : view.text;
  };
