...
function normalizedModelId(model) {
  ...
  if (id === "deepseek-chat" || id === "deepseek-reasoner" || id === "deepseek-flash") return "deepseek-v4-flash";
  return id;
}
...
function formatResetAt(iso, now) {
  const date = new Date(iso);
  const timestamp = date.getTime();
  if (!Number.isFinite(timestamp)) return "\u672A\u77E5";
  const pad = (value) => String(value).padStart(2, "0");
  const time = `${pad(date.getHours())}:${pad(date.getMinutes())}`;
  return timestamp - now > 0 && timestamp - now < 24 * 60 * 60 * 1e3 ? time : `${date.getMonth() + 1}\u6708${date.getDate()}\u65E5${time}`;
}
...
function compactProviderUsage(provider, payload, now = Date.now()) {
  ...
  if (view.kind === "subscription") {
    ...
    return best === void 0 ? `${view.label} \u6682\u65E0` : `${view.label} \u7528\u91CF ${best.percent}%`;
  }
  if (view.kind === "text") {
...
function detailedProviderUsage(provider, payload, now = Date.now()) {
  ...
  if (view.kind === "subscription") {
    return { kind: "subscription", label: view.label, windows: view.windows };
  }
